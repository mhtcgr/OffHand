...
class MultiShotEndActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val shots = intent.getStringExtra("SHOTS") ?: "0"

        // 发送数据到手表（新增代码）
        sendTrainingData(score, shots)
    ...
    private fun sendTrainingData(score: String, shots: String) {
        try {
            val outputStream = DeviceScanActivity.connectedSocket?.outputStream
            outputStream?.apply {
                write("score:$score\n".toByteArray())
                write("shots:$shots\n".toByteArray())
                flush()
                Log.d("Bluetooth", "训练数据已发送")
            }
        } catch (e: IOException) {
